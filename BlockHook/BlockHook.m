...
@implementation NSInvocation (BlockHook)

// Thanks to the ReactiveCocoa team for providing a generic solution for this.
- (id)blookHook_argumentAtIndex:(NSUInteger)index {
    const char *argType = [self.methodSignature getArgumentTypeAtIndex:index];
    
    NSString *typeStr = [NSString stringWithUTF8String:argType];
    if ([typeStr containsString:@"\""]) {
        argType = [typeStr substringToIndex:1].UTF8String;
    }
...
- (NSArray *)blockHook_arguments {
    NSMutableArray *argumentsArray = [NSMutableArray array];
    for (NSUInteger idx = 1; idx < self.methodSignature.numberOfArguments; idx++) {
        [argumentsArray addObject:[self blookHook_argumentAtIndex:idx] ?: NSNull.null];
    }
